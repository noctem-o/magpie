...
constitutional owner choice only: no Rust or Python profile implementation,
permanent corpus, profile manifest, or Go conformer had landed at this
annotation point. A-021 therefore remains Confirmed and a pre-alpha blocker
for a portable-verification claim. The next A-021 tranche is the exact
candidate normative portable corpus/profile manifest.
...
| A-021 signature semantics | PR #121 supplies permissive-relation evidence and hostile witnesses. Accepted ADR-0010 now selects the additive `magpie-ed25519-canonical-prime-subgroup-v1` profile, but no conformer implements it. | **PRE-ALPHA BLOCKER** for a portable-verification claim | The owner choice is resolved, but A-021 is not remediated. The next tranche is the exact candidate normative portable corpus/profile manifest; do not treat ratification or that corpus as cross-language realization. |
