...
| A-021 | The strict portable Ed25519 subprofile is ratified, but no conformer implements or proves it. | Confirmed | conditional; high; trust-root/profile | Accepted ADR-0010 selects `magpie-ed25519-canonical-prime-subgroup-v1` while preserving existing unprofiled Rust/Python behavior as compatibility-only. The trust root remains externally supplied; current runtime paths have not changed. | Owner ratification on 2026-08-21 resolves the profile decision only. The exact corpus and independent Rust/Python/Go conformance evidence remain absent. | verifier/tooling contract |
...
| A-021 | Ratified strict Ed25519 profile remains unimplemented and unconformed | F20 | Confirmed | Owner choice is fixed by Accepted ADR-0010; runtime and cross-language evidence remain absent. |
...
- **Weak-root/strict-verification implementation gap — A-021.** The external
  ...
  Python behavior remains unchanged and compatibility-only. The next evidence
  boundary is the exact corpus followed by independent conformer realization.
