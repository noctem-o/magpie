...
positive effect:
the Ticket 0056 prerequisite is met by construction — evidence nodes
can no longer choose the bytes a digest evaluation checks
```
...
3. The canonical statement carries the descriptor; the claim content
   hash remains the statement hash, never a subject digest.
...
binding:         statement == typed statement == derived canonical
                 statement; content_hash == SHA-256(statement)
```
...
unrelated bytes — is closed by construction: no evidence or edge field
may carry bytes an evaluation could consume.
...
and standing policies v0-v3 remain byte- and behavior-identical. The new
...
replay; strictly parse the descriptor with closed failures (malformed,
duplicate, unknown, missing, wrong-typed, unknown schema or predicate,
invalid hex, oversize); derive and three-way-check the canonical
statement and content hash; decode the subject from the claim only; and
only then evaluate, through one shared path for both polarities. Every
prerequisite failure is a closed audit failure with no standing effect
and no negative meaning. Subject resolution grants no standing.
...
- Every authority-bearing value is private-construction,
  ...
  material only.
- Evidence attestation may carry routing fields only; any byte-carrying
  evidence field is rejected, never compared.
...
| --- | --- | --- | --- | --- |
| Byte equality is the decisive identity relation; the canonical statement must carry the complete subject descriptor; content_hash stays statement-hash | A, B | accepted | v0 statement carries only the digest; content_hash = SHA-256(statement); metadata alone is not subject-canonical | §5-§6 law and binding |
...
| One resolution path for both polarities; failures never false; evidence carries no bytes | A, B | accepted | the Ticket 0056 failure mode was exactly a second (negative) byte source | §5, §8, §9 |
...
- **Stop conditions**: no direct-refutation contract may proceed
  ...
  authority path, missing/malformed/unavailable never false, no byte
  source in evidence, and brand-new schema/predicate identities leaving
...
any byte source in evidence or edges; any caller-substitutable authority;
...
metadata action is authorised. The user retains sole publication and
merge authority.
...
2. The subject bytes come only from the claim; no evidence or edge byte
   source exists anywhere.
3. The canonical statement carries the complete subject descriptor and
   the three-way binding is exact.
4. `sha256_bytes_equals_v0`, its parser and trace, policies v0-v3, L0,
   and FORMAT are untouched; the new schema is fail-closed to v0.
5. Missing, malformed, oversized, or unparseable subject material is
   never falsity.
6. No predicate, policy, standing rule, refutation, resolver, receipt,
   or runtime is ratified or claimed.
7. Private-construction and no-caller-receipt requirements hold for
   every authority-bearing value.
8. The external and replay-owned extensions are recorded as unratified,
   with prerequisites and stop conditions.
9. The Ticket 0056 counterexample is answered by construction.
