...
1. Make Magpie usable: land and use the `magpie` CLI (`crates/magpie-cli`). It stores JSONL
   through `FileStore`, because SQLite L0 has no replay into projections until A-007 is settled.
2. Dogfood it: import the audit findings and the manuscript claims (dry run first; the owner runs real imports).
3. Read-only MCP librarian (`magpie mcp`: search, show, why, frontier), per the librarian contracts.
4. Owner decisions pending: currentness resolver (ADR-0006 subset), fallible projections (A-007)
   and with them a SQLite L0 replay path, a claim domain for mathematical results, and a
   minimal `EpistemicGate` for agent proposals.
5. Paused until the CLI is in daily use: the C3–C5 public pre-alpha release-assurance queue.
6. Still deferred: ingestion/CAS, governed agent writing, ADR-0007 authority runtime, effectful MCP, production KMS.

What landed before this list, with PR references, is in `CHANGELOG.md`.
